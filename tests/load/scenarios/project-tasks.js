// tests/load/scenarios/project-tasks.js
// Cenário: Tasks do Kanban — GET /api/projects/:id/tasks
// Tipo: Frequência alta | Auth: Sim | SLO: p95 < 500ms
//
// Variável extra:
//   LOAD_TEST_PROJECT_ID - ID de projeto real para o teste (obrigatório para resultados reais)
//
// Uso:
//   k6 run --env LOAD_TEST_PROJECT_ID=proj-uuid tests/load/scenarios/project-tasks.js
...
const latencyTrend = new Trend('project_tasks_latency')
...
export default function (data) {
  ...
  const res = http.get(`${BASE_URL}/projects/${LOAD_TEST_PROJECT_ID}/tasks`, { headers })

  latencyTrend.add(res.timings.duration)

  const ok = check(res, {
    'tasks status 200': (r) => r.status === 200,
    'tasks latência < SLO p95': (r) => r.timings.duration < SLO_P95,
    'tasks retorna array': (r) => {
      if (r.status !== 200) return true // skip body check on auth fail
      try {
        const body = JSON.parse(r.body)
        return Array.isArray(body) || Array.isArray(body.data) || Array.isArray(body.tasks)
      } catch {
        return false
      }
    },
