...
export function AILoadingState({ module, message, className }: AILoadingStateProps) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm text-gray-600 dark:text-gray-300 font-medium min-w-0">
        {displayMessage}
      </span>
      <div className="flex items-center gap-1 ml-1" aria-hidden="true">
        <span className="h-2 w-2 rounded-full bg-indigo-500 dark:bg-indigo-400 animate-pulse [animation-delay:0ms]" />
        <span className="h-2 w-2 rounded-full bg-indigo-500 dark:bg-indigo-400 animate-pulse [animation-delay:150ms]" />
        <span className="h-2 w-2 rounded-full bg-indigo-500 dark:bg-indigo-400 animate-pulse [animation-delay:300ms]" />
      </div>
    </div>
  )
}
